...
def ai(p):
    if not MISTRAL_API_KEY: return "Mistral API key not set."
    return Mistral(api_key=MISTRAL_API_KEY).chat.complete(model="mistral-tiny", messages=[{"role": "user", "content": p}]).choices[0].message.content

def tr(t):
    if not MISTRAL_API_KEY: return "Mistral API key not set."
    return Mistral(api_key=MISTRAL_API_KEY).chat.complete(model="mistral-tiny", messages=[{"role": "user", "content": f"Translate to English: {t}"}]).choices[0].message.content
...
async def stc(msg: types.Message, st: FSMContext, cmd: CommandObject):
    ...
    await msg.answer(
        ...
        "━━━━━━━━━━━━━━━━━━━\n"
        "🧠 Grok | 🎨 NanoBanana | 👁️ Trocr | 💎 Stable Diffusion XL\n"
        "🎯 Mistral AI | 🔮 Claude 3 | 🌪️ Flux Pro | 🤖 ChatGPT\n"
        "━━━━━━━━━━━━━━━━━━━\n\n"
        "✨ Что я умею:\n"
...
async def sub_ck(cb: types.CallbackQuery, st: FSMContext):
    if await cs(cb.from_user.id):
        await cb.message.delete()
        await cb.message.answer("✅ Готово!", reply_markup=mk)
        await stc(cb.message, st, CommandObject(args=""))
...
async def vip_cmd(msg: types.Message):
    await msg.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⭐ Stars (40)", pay=True)],
...
async def md_cht(msg: types.Message, st: FSMContext): await st.set_state(UM.wc); await msg.answer("💬 Чат активирован.")
...
async def h_tr(msg: types.Message, st: FSMContext):
    await msg.answer(f"🌐 <pre>{tr(msg.text)}</pre>", parse_mode="HTML")
    await st.clear()
...
async def h_txt(msg: types.Message, st: FSMContext):
    ...
    fp = f"Напиши {msg.text}. Сделай текст качественным, грамотным и интересным. Объём: 300-500 слов."
    txt = ai(fp)
    if txt:
        if len(txt) > 4000:
            for i in range(0, len(txt), 4000): await msg.answer(txt[i:i+4000])
