...
@dp.message(F.text)
async def debug_text(msg: types.Message, state: FSMContext):
    # Показывает, что бот получил сообщение, и сбрасывает состояние, чтобы не застревать в FSM
    current_state = await state.get_state()
    if current_state:
        await state.clear()
    await msg.answer(f"✅ Бот работает! Получено: «{msg.text}»\n(Отладочный режим. Остальные функции временно отключены.)")
