...
def gi(p):
    # Генерация изображений временно отключена
    return None
...
def ai(p):
    if not OPENROUTER_API_KEY: return "❌ Ключ OpenRouter не установлен."
    try:
...
async def stc(msg: types.Message):
    ...
    await msg.answer(
        ...
        "🧠 DeepSeek | 🎨 SD | 👁️ Trocr | 🌐 + многое другое\n"
        "━━━━━━━━━━━━━━━━━━━\n\n"
        "✨ Что я умею:\n🎨 Картинки (временно отключены) | 📷 Распознавание | 💬 Чат | 🌐 Перевод | ✍️ Тексты\n\n"
...
async def handle_all(msg: types.Message):
    ...
    if msg.text == "🎨 Генерация":
        ...
        await msg.answer("🎨 Генерация изображений временно недоступна. Попробуйте другие функции!")
        return
    ...
    if mode == 'image':
        await msg.answer("🎨 Генерация изображений временно недоступна.")
