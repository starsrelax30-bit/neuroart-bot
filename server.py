...
def gi(p):
    ...
    pl = {"inputs": fp}
    try:
        r = requests.post(IMAGE_API_URL, headers=HEADERS, json=pl, timeout=45)
        if r.status_code == 200: return r.content
        if 'loading' in r.text.lower() or r.status_code == 503:
            tm.sleep(25)
            r = requests.post(IMAGE_API_URL, headers=HEADERS, json=pl, timeout=45)
            if r.status_code == 200: return r.content
        logging.error(f"HF image error: {r.status_code} - {r.text[:300]}")
    except Exception as e:
        logging.error(f"HF image exception: {e}")
    return None
...
async def handle_all(msg: types.Message):
    ...
    if mode == 'image':
        ...
        if img:
            ...
            await m.edit_text("❌ Ошибка генерации. Попробуйте позже.")
            lr(uid, msg.from_user.username, 'image', 0)
