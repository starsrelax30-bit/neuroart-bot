...
async def adm_cmd(msg: types.Message, state: FSMContext):
    if not ia(msg.from_user.id): return
    await state.set_state(AdminState.waiting_for_action)
    await msg.answer("🔧 Админ-панель", reply_markup=admin_kb)

@dp.message(AdminState.waiting_for_action, F.text == "📊 Статистика")
async def a_st(msg: types.Message, state: FSMContext):
    s = gs()
    await msg.answer(f"📊 Запросов: {s['total_requests']}\n👥 Пользователей: {s['unique_users']}\n✅ Успешных: {s['successful']}\n💎 VIP: {s['vips']}\n🚫 Банов: {s['bans']}")

@dp.message(AdminState.waiting_for_action, F.text == "👥 Пользователи")
async def a_us(msg: types.Message):
    u = gru(10)
    await msg.answer("\n".join([f"• `{uid}` — @{un or 'нет'}" for uid, un in u]) or "Нет данных.", parse_mode="Markdown")

@dp.message(AdminState.waiting_for_action, F.text.in_(["💎 VIP", "🚫 Бан", "✅ Разбан", "➕ Админ", "➖ Админ"]))
async def a_prompt_id(msg: types.Message, state: FSMContext):
    action = msg.text
    await state.update_data(admin_action=action)
    await state.set_state(AdminState.waiting_for_id)
    await msg.answer("Введите ID пользователя:")

@dp.message(AdminState.waiting_for_action, F.text == "📢 Рассылка")
async def a_prompt_broadcast(msg: types.Message, state: FSMContext):
    await state.set_state(AdminState.waiting_for_broadcast)
    await msg.answer("Введите текст рассылки:")

@dp.message(AdminState.waiting_for_id)
async def a_process_id(msg: types.Message, state: FSMContext):
    uid = int(msg.text) if msg.text.strip().isdigit() else None
    if not uid:
        await msg.answer("❌ Неверный ID.")
        return
    data = await state.get_data()
    action = data.get('admin_action')
    if action == "💎 VIP": sv(uid); await msg.answer(f"💎 {uid} → VIP")
    elif action == "🚫 Бан": ban(uid); await msg.answer(f"🚫 {uid} забанен")
    elif action == "✅ Разбан": ubn(uid); await msg.answer(f"✅ {uid} разбанен")
    elif action == "➕ Админ": aa(uid); await msg.answer(f"✅ {uid} → админ")
    elif action == "➖ Админ": ra(uid); await msg.answer(f"➖ {uid} → не админ")
    await state.set_state(AdminState.waiting_for_action)
...
async def a_broadcast(msg: types.Message, state: FSMContext):
    ...
    cnt = 0
    for (uid,) in us:
        try: await bot.send_message(uid, f"📢 {text}"); cnt += 1
        except: pass
    await msg.answer(f"📢 Отправлено {cnt} получателям.")
    await state.set_state(AdminState.waiting_for_action)

@dp.message(AdminState.waiting_for_action, F.text == "🔙 Выход")
async def a_exit(msg: types.Message, state: FSMContext):
    await state.clear()
    await msg.answer("🔧 Выход из админ-панели.", reply_markup=mk)
